...
function msgStyle(screen: Awaited<ReturnType<typeof render>>) {
    return (
        screen.getByRole('listitem').first().element()
...
function renderWithTimestamps(logs: LogEntry[]) {
    ...
    const Wrapper = defineComponent({
        components: { Log },
        props: { logs: { type: Array as PropType<LogEntry[]>, required: true } },
        setup() {
            return { logRef };
        },
        mounted() {
...
describe('Log', () => {
    ...
    describe('sorting and limiting', () => {
        it('renders log entries in descending timestamp order', async () => {
            const logs: LogEntry[] = [
                makeLog({ id: 1, message: 'First',  timestamp: new Date('2024-01-01T10:00:00') }),
                makeLog({ id: 2, message: 'Second', timestamp: new Date('2024-01-01T11:00:00') }),
                makeLog({ id: 3, message: 'Third',  timestamp: new Date('2024-01-01T12:00:00') }),
            ];
            const screen = await render(Log, { props: { logs } });
        ...
        it('displays at most 25 entries when more are provided', async () => {
            const logs: LogEntry[] = Array.from({ length: 30 }, (_, i) =>
                makeLog({ id: i, message: `Entry ${i}`, timestamp: new Date(i * 1000) }),
            );
            const screen = await render(Log, { props: { logs } });
            await screen.getByRole('button').click();
