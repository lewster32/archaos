...
describe('UnitStats', () => {
    describe('movement / flying icon', () => {
        it('shows the movement stat title when not flying', async () => {
            const screen = await render(UnitStats, { props: { unit: makeUnit() } });
    ...
    describe('ranged combat stats', () => {
        ...
        it('shows ranged combat stat when rcm is non-zero', async () => {
            const screen = await render(UnitStats, {
                props: {
                    unit: makeUnit({ properties: { mov: 2, com: 3, rcm: 4, rng: 6, def: 4, mnv: 3, res: 2 } }),
                },
            });
    ...
    describe('status badges', () => {
        it('shows Wizard badge for wizard units', async () => {
            const screen = await render(UnitStats, { props: { unit: makeUnit({ wizard: true }) } });
        ...
        it('shows Dead badge for dead units', async () => {
            const screen = await render(UnitStats, { props: { unit: makeUnit({ dead: true }) } });
    ...
    describe('itemNumClass', () => {
        it('applies the correct numeral class for stats 1–10', async () => {
            for (let n = 1; n <= 10; n++) {
                const screen = await render(UnitStats, {
                    props: {
                        unit: makeUnit({
                            properties: { mov: n, com: n, rcm: 0, rng: 0, def: n, mnv: n, res: n },
                        }),
                    },
                });
                await expect
        ...
        it('applies no numeral class for stat value 0', async () => {
            const screen = await render(UnitStats, {
                props: { unit: makeUnit({ properties: { mov: 0, com: 0, rcm: 0, rng: 0, def: 0, mnv: 0, res: 0 } }) },
            });
            await expect
        ...
        it('applies no numeral class for stat value 11 or above', async () => {
            const screen = await render(UnitStats, {
                props: { unit: makeUnit({ properties: { mov: 11, com: 11, rcm: 0, rng: 0, def: 11, mnv: 11, res: 11 } }) },
            });
            await expect
