...
function makeMockImage() {
    ...
    return {
        ...
        setData: (_key: string, value: unknown) => { store[_key] = value; return {}; },
        destroy: vi.fn(),
        x: 0,
        y: 0,
        depth: 0,
...
describe('Wizard instance methods', () => {
    ...
    describe('kill', () => {
        it('plays death sounds, destroys the sprite, and resolves after checkWinCondition', async () => {
            ...
            expect((board as any).sound.play).toHaveBeenCalledWith('deadwizard1');
            expect((board as any).sound.play).toHaveBeenCalledWith('disbelieve');
