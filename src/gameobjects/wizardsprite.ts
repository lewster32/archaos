...
export class WizardSprite extends Phaser.GameObjects.Sprite {
    ...
    protected generateFrames() {
        ...
        WizardSprite.replaceColor(canvas, searchColors.primaryDark, replaceColors[this._wizCode.pri].dark);
        WizardSprite.replaceColor(canvas, searchColors.primaryMid, replaceColors[this._wizCode.pri].mid);
        WizardSprite.replaceColor(canvas, searchColors.primaryLight, replaceColors[this._wizCode.pri].light);

        WizardSprite.replaceColor(canvas, searchColors.secondaryDark, replaceColors[this._wizCode.sec].dark);
        WizardSprite.replaceColor(canvas, searchColors.secondaryMid, replaceColors[this._wizCode.sec].mid);
        WizardSprite.replaceColor(canvas, searchColors.secondaryLight, replaceColors[this._wizCode.sec].light);

        WizardSprite.replaceColor(canvas, searchColors.skinMid, replaceSkin[this._wizCode.skin].mid);
        WizardSprite.replaceColor(canvas, searchColors.skinLight, replaceSkin[this._wizCode.skin].light);
    ...
    static replaceColor(canvas: Phaser.Textures.CanvasTexture, searchColor: number[], replaceColor: number[]): Phaser.Textures.CanvasTexture {
        ...
        for (let i = 0; i < imgData.data.length; i += 4) {
            if (imgData.data[i + 3] === 255) {
                if (imgData.data[i] === searchColor[0] && imgData.data[i + 1] === searchColor[1] && imgData.data[i + 2] === searchColor[2]) {
