...
function makeSpell(id: number, opts: { persist?: boolean; castTimes?: number } = {}): Spell {
    ...
    return {
        id,
        persist,
        get castTimes() { return _castTimes; },
        set castTimes(v) { _castTimes = v; },
        resetCastTimes: vi.fn(),
        get owner() { return undefined as any; },
        set owner(_: any) { /* no-op — avoids the real Spell.owner validation */ },
    } as unknown as Spell;
}
...
describe('Player', () => {
    ...
    describe('local player basics', () => {
        ...
        it('has a wizcode from the config', () => {
            const p = new Player(mockBoard, 5, { name: 'Charlie', type: GameSetupPlayerType.Local, wizcode: '0000000000' }, 0x0000ff);
    ...
    describe('spell management', () => {
        ...
        describe('casting piece interaction', () => {
            ...
            it('throws if the piece owner is not this player', () => {
                const otherPlayer = new Player(mockBoard, 2, { name: 'Eve', type: GameSetupPlayerType.Local }, 0x0000ff);
                const piece = { id: 1, owner: otherPlayer } as any;
                expect(() => player.castingPiece = piece).toThrow('Cannot set casting piece to a piece not owned by this player');
            });
    ...
    describe('computer player', () => {
        it('creates an AI controller when type is Computer', () => {
            const player = new Player(mockBoard, 2, {
                name: 'CPU',
