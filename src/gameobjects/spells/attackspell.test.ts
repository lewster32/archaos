...
function makeMockPiece(opts: {
    ...
    const {
        id = 1, type = UnitType.Creature, owner = null, dead = false,
        currentMount = null, engulfed = false, canBeMagicAttacked = true,
        x = 0, y = 0, fullName = 'Test Piece', wizardStatus = false,
    } = opts;
    return {
        id, type, owner, dead, currentMount, engulfed, canBeMagicAttacked,
...
describe('AttackSpell.doCast', () => {
    ...
    beforeEach(() => {
        ...
        enemy = makeMockPiece({ owner: { id: 99, name: 'Enemy' }, canBeMagicAttacked: true });
    ...
    it('throws when targets list is empty', async () => {
        const s = new AttackSpell(board, 1, makeAttackConfig());
        s.owner = owner;
        await expect(s.doCast(owner, castingPiece, new Geom.Point(0, 0), [])).rejects.toThrow('No targets');
    });
    ...
    it('throws when no target passes getValidTarget', async () => {
        ...
        const s = new AttackSpell(board, 1, makeAttackConfig());
        s.owner = owner;
        await expect(s.doCast(owner, castingPiece, new Geom.Point(0, 0), [enemy])).rejects.toThrow('No valid target');
    });

    it('returns true on a successful attack', async () => {
        const s = new AttackSpell(board, 1, makeAttackConfig());
        s.owner = owner;
        const result = await s.doCast(owner, castingPiece, new Geom.Point(0, 0), [enemy]);
    ...
    describe('Lightning projectile', () => {
        ...
        beforeEach(() => {
            spell = new AttackSpell(board, 1, makeAttackConfig({ projectile: UnitRangedProjectileType.Lightning }));
            spell.owner = owner;
        });
        ...
        it('plays "lightningexplode" hit sound', async () => {
            await spell.doCast(owner, castingPiece, new Geom.Point(0, 0), [enemy]);
            expect((board as any).sound.play).toHaveBeenCalledWith('lightningexplode');
        });
    ...
    describe('Magic Bolt projectile', () => {
        let spell: AttackSpell;
        beforeEach(() => {
            spell = new AttackSpell(board, 1, makeAttackConfig({ projectile: UnitRangedProjectileType.MagicBolt }));
            spell.owner = owner;
        });
        ...
        it('plays "magicboltexplode" hit sound', async () => {
            await spell.doCast(owner, castingPiece, new Geom.Point(0, 0), [enemy]);
            expect((board as any).sound.play).toHaveBeenCalledWith('magicboltexplode');
        });
    });
    ...
    describe('Justice projectile', () => {
        let spell: AttackSpell;
        beforeEach(() => {
            spell = new AttackSpell(board, 1, makeAttackConfig({ projectile: UnitRangedProjectileType.Justice }));
            spell.owner = owner;
        });
        ...
        it('does not play a beam sound (no beamSound for Justice)', async () => {
            ...
            const playCalls = ((board as any).sound.play as any).mock.calls.map((c: any[]) => c[0]);
    ...
    describe('Dark Power projectile', () => {
        let spell: AttackSpell;
        beforeEach(() => {
            spell = new AttackSpell(board, 1, makeAttackConfig({ projectile: UnitRangedProjectileType.DarkPower }));
            spell.owner = owner;
        });
    ...
    describe('destroyWizardCreatures', () => {
        ...
        beforeEach(() => {
            wizardOwner = { id: 99, name: 'Enemy Wizard', destroyCreations: vi.fn().mockResolvedValue(undefined) };
            wizardTarget = makeMockPiece({ owner: wizardOwner, type: UnitType.Wizard, wizardStatus: true, fullName: 'Gandalf' });
            wireEnemy(board, wizardTarget);
            spell = new AttackSpell(board, 1, makeAttackConfig({
        ...
        it('calls destroyCreations when roll succeeds on a wizard target', async () => {
            await spell.doCast(owner, castingPiece, new Geom.Point(0, 0), [wizardTarget]);
        ...
        it('kills a non-wizard target normally even when destroyWizardCreatures is set', async () => {
            const creature = makeMockPiece({ owner: { id: 99 }, canBeMagicAttacked: true });
            wireEnemy(board, creature);
            await spell.doCast(owner, castingPiece, new Geom.Point(0, 0), [creature]);
